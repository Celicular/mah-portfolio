    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm font-sans text-white overflow-y-auto">
      <div className="w-full max-w-2xl bg-[#111113] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form id="item-form" onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input required type="text" name="title" value={formData.title || ''} onChange={handleChange} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
            </div>

            {type === 'portfolio' ? (
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-white/70">Category</label>
                <input required type="text" name="category" value={formData.category || ''} onChange={handleChange} placeholder="e.g. React & PostgreSQL" className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
              </div>
            ) : (
              <>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-white/70">Type</label>
                  <input required type="text" name="type" value={formData.type || ''} onChange={handleChange} placeholder="e.g. AI / NLP Moderation System" className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-white/70">Tech Stack (comma separated)</label>
                  <input required type="text" name="tech" value={displayTech} onChange={handleTechChange} placeholder="React, Node.js, Python" className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
                </div>
              </>
            )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input required type="text" name="image" value={formData.image || ''} onChange={handleChange} placeholder="https://..." className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea required name="desc" value={formData.desc || ''} onChange={handleChange} rows={4} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500 resize-none" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-white/70">External Link</label>
              <input required type="url" name="link" value={formData.link || ''} onChange={handleChange} placeholder="https://..." className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-sky-500" />
            </div>

            <div className="flex items-center gap-3 pt-2">
              <input type="checkbox" id="under_development" name="under_development" checked={formData.under_development || false} onChange={(e) => setFormData(prev => ({...prev, under_development: e.target.checked}))} className="w-5 h-5 rounded bg-black/50 border-white/10 text-sky-600 focus:ring-sky-500 focus:ring-offset-[#111113] cursor-pointer" />
        ...
        <div className="p-6 border-t border-white/10 shrink-0 flex justify-end gap-3 bg-[#111113] rounded-b-2xl">
          ...
          <button form="item-form" type="submit" className="px-5 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 text-white transition-colors font-medium shadow-lg shadow-sky-600/20">
